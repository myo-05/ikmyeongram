    ...
    if request.method == 'GET':  # 회원가입 페이지를 눌렀을 때
        ...
        if username and password and password2 and nickname and email :
            if password != password2:
                return render(request, 'user/signup_detail.html',{'error_message': '[비밀번호불일치!] 진정하고 천천히 다시써봐요 예?'})
            else:
                exist_user = get_user_model().objects.filter(username=username)
                if exist_user:
                    # 중복된 ID라면 회원가입창으로 돌아가기
                    # 영오: 이미 있는 ID입니다. 새로 지정해주세요. 메세지 떠야합니다.
                    return render(request, 'user/signup_detail.html', {'error_message': '[중복 발견!] 이미 있는 ID입니다. 아시겠어요?'})
                else:
                    # 유저계정생성하여 DB저장
                    # 영오: 회원가입완료. 메세지 떠야합니다.
            ...
            return render(request, 'user/signup_detail.html',{'error_message': '[빈칸 발견!] 공란이 빤히 보이는데, 미치셨어요?'})
